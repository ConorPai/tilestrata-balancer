...
NodeList.prototype.syncNodeLayers = function(ipaddr, body, callback) {
	if (!body.id) return callback(new Error('No "id" provided (unique instance id)'));
	if (!body.listen_port) return callback(new Error('No "listen_port" provided'));
	var self = this;
	var id = body.id;
	...
	if (existing_id == undefined) {
		this.register(ipaddr, body, callback);
		return;
	}
	...
	body.layers.forEach(function(layer) {
		...
		if (self.layers_by_id[id].indexOf(layer.name) == -1) {
			self.layers_by_id[id].push(layer.name);
			self.layer_options[layer.name] = layer.options;
	...
	log.info('pool', 'synchronized node layers ' + target);
	callback(null, true);
};
